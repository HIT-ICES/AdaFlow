...
processing_data_path = "DS1/glove.6B/processing_data/"
output_data_path = "DS1/glove.6B/output_data/"
...
def generate_data_onlyusemashup():
    ...
    for mashup_name in mashup_dict:
        ...
        if mashup_name not in mashup_col:
            ...
            # mashup_info.csv
            mashup_csv.writerow([
                mashup_col[mashup_name], mashup['url'], mashup_name, " ".join(mashup_filtered_tokens), mashup['mashup_type'],
                ' '.join(mashup['categories']), mashup['date']
            ])

    new_api_dev_embeds = np.empty((num_api, 300), dtype=float)
    for index in range(len(api_dev_embeds)):
        new_api_dev_embeds[index] = api_dev_embeds[index].numpy()
    # api_dev_embeds.npz
    api_dev_embeds = np.array(new_api_dev_embeds)
    np.savez(os.path.join(output_data_path, 'api_dev_embeds.npz'), new_api_dev_embeds)
    # tag_embeds.npz
    new_tag_embeds = np.empty((num_tag, 300), dtype=float)
    for index in range(len(tag_embeds)):
        new_tag_embeds[index] = tag_embeds[index].numpy()
    tag_embeds = np.array(tag_embeds)
    np.savez(os.path.join(output_data_path, 'tag_embeds.npz'), new_tag_embeds)
    # mashup_dev_embed.npz
    new_mashup_dev_embeds = np.empty((num_mashup, 300), dtype=float)
    for index in range(len(mashup_dev_embeds)):
        new_mashup_dev_embeds[index] = mashup_dev_embeds[index].numpy()
    mashup_dev_embeds = np.array(mashup_dev_embeds)
    np.savez(os.path.join(output_data_path, 'mashup_dev_embeds.npz'), new_mashup_dev_embeds)
    #
